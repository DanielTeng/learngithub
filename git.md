...
+ 当前坐在feature2分支上，迁基到feature1分支上。
...
+ git rebase --skip (解决迁基冲突，这会放弃当前就是迁基的分支做的变更，以基底为准，自然没有了冲突，迁过去的，是以patch处理)
+ 迁基的时候，HEAD处于游离状态。
+ 迁基会导致迁移的分支上的commit_id的重新生成，按规则。但先后依赖关系不变。所以往往只需要解决迁移点链接的冲突。
+ 迁基导致提交历史的直线，简洁。
